...
| 실험 | 방법 | Dev Accuracy | Test Accuracy | Baseline 대비 |
|---|---|---:|---:|---:|
| Exp1 | BoW Baseline | - | 67.38% | - |
| Exp2 | BoW + Sweep | - | 68.26% | +0.88%p |
| Exp3-v8 | TF-IDF + Sweep + 추가 개선 | **70.05%** | **69.13%** | **+1.75%p** ★ |
| Exp4 | GloVe300 + Sweep | - | 62.00% | -5.38%p |
| Exp5 | MiniLM + Sweep | - | 66.07% | -1.31%p |
...
- **충족**
- 근거: Exp2(68.26%), Exp3-v8(69.13%)가 baseline(67.38%)보다 높음
...
### Exp3-v8. TF-IDF + Sweep + 추가 개선 (최종 제출)
- 변경 이유: 단문 감성 데이터에서 단어 중요도(IDF) 반영이 유리; 추가 전처리·피처 엔지니어링으로 성능 극대화
- 핵심 변경: `max_features=30000`, `min_df=2`, 슬랭·부정어 전처리, 핸드크래프트 피처(6차원) 결합, sweep 재탐색
- 결과: Dev 70.05% | Test 69.13% (+1.75%p)
- 해석: 과제 제약(MLP 고정) 내 최고 성능; 전처리+피처+sweep 조합이 가장 효과적
...
- Dev 70.05%에 도달하였으나 dev-test gap(~0.9%p)이 소폭 존재함
...
2. 최종 제출 모델: `Exp3-v8` (Dev 70.05% | Test **69.13%**, +1.75%p)
3. 기본 5개 실험 중 최고 성능: `Exp2 BoW + Sweep` (Test 68.26%)
4. 데이터 특성상 단순 키워드 기반 신호를 잘 활용하는 TF-IDF + 추가 개선이 가장 효율적
5. 사전학습 임베딩(GloVe/MiniLM/MPNet)은 MLP 고정 제약 아래에서 오히려 성능 저하

---

## 7. 제출 체크리스트

- [x] `W5_HW1_PracticalNLP_202200274_고민서.ipynb` 저장 (출력 포함)
- [x] 최고 성능 모델 코드 포함 (Exp3-v8, Test 69.13%)
- [x] 5개 실험 구현 내용 + 성능 + 간략 분석 포함
- [x] 모델 구조(MLP) 변경하지 않았음을 보고서에 명시
- [x] `W5_HW2_PracticalNLP_202200274_고민서.ipynb` 감성 예측 구현 완료
